...
if __name__ == '__main__':
    ...
    if chunk_amount == chunk_idx_future: 
        ...
        exit()
    future_train_images = train_images[chunk_idx_future*train_chunk_size:(chunk_idx_future+1)*train_chunk_size]
    test_actions = list(dict.fromkeys([e["file_name"].split("/")[2] for e in future_train_images]))
    ...
    if args.baseline: 
        filename = os.path.join(checkpoint_dir, "resactionval_base_dist.csv")
    else: 
        filename = os.path.join(checkpoint_dir, "resactionval_dist.csv")
