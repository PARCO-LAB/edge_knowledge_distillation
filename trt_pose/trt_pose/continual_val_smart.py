...
import glob
from parcopose_from_folder import H36M_2D_COLS, skeletons_to_row, FRAMERATE, H36M_2D_COLS_NO_ACC
...
def find_eigen_vectors_number(data, threshold):
    data = data[H36M_2D_COLS_NO_ACC]
    data = np.nan_to_num(data)
    print(data.shape)
    
    _, S, _ = np.linalg.svd(data.T, full_matrices=False)  # Calcola la SVD

    cumulativeSum = np.cumsum(S) / np.sum(S)
    print( cumulativeSum.shape)
    numElementsNeeded = np.argmax(cumulativeSum >= threshold) + 1
    
    return numElementsNeeded
...
if __name__ == '__main__':
    ...
    if args.baseline or chunk_idx_input == -1: 
        print("Using default parco model")
        dnn = DNN(kind="densenet", suffix="parco", 
                  enable_opt=True).load()
    else:
        print("Using chunk_{}_{}.pth parco model".format(args.selector, chunk_idx_input))
        dnn = DNN(kind="densenet", suffix="parco", 
                  model_fp=os.path.join(checkpoint_dir, 'chunk_{}_{}.pth'.format(args.selector, chunk_idx_input)), 
                  enable_opt=True).load()
    ...
    if args.baseline: 
        filename = os.path.join(checkpoint_dir, "resval_base_{}.csv".format(args.selector))
    else: 
        filename = os.path.join(checkpoint_dir, "resval_{}.csv".format(args.selector))
    ...
    if os.path.exists(filename):
        ...
        data = pd.DataFrame()
    n_eigen = find_eigen_vectors_number(inference_data, 0.99)
    inference_data['eigen'] = n_eigen
